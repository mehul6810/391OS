...
/* page directory structure that contains all the info about page */
/* some notes:
   Avail: not gonna be used
   G: if 1, TLB won't clear these addresses won't be flushed
      set to 1 only for kernel page
      ignored for 4KB PDEs
   PS: if 0, 4KB PDE. if 1, 4MB PDE
   A: not gonna be used
   PCD: if 0, processor won't cache memory
        should be 1 for program code, kernel pages, and program pages
        should be 0 for video memory
   PWT: should always be 0
   U/S: if 1, memory mapped is user-level ( privilege leves 0-3 can access)
        if 0, supervisor-only (privilege levels 0-2 can access)
        1 for all user-level page_address. 0 for all kernel pages
   R/W: if 1, pages are read/write. if 0, read-only
        set all pages to 1
   P: if 1, PDE valid. if 0, PDE/PTE does not exist
      all valid PDEs set to 1
   PAT: set to 0
   D: set to 0
   Reserved: set to 0 */
